...
func (vq *VideoQuery) QuerySubtitles() *SubtitleQuery {
	query := (&SubtitleClient{config: vq.config}).Query()
...
func (vq *VideoQuery) Clone() *VideoQuery {
	...
	return &VideoQuery{
		...
		withSubtitles: vq.withSubtitles.Clone(),
...
func (vq *VideoQuery) WithSubtitles(opts ...func(*SubtitleQuery)) *VideoQuery {
	query := (&SubtitleClient{config: vq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	vq.withSubtitles = query
	return vq
}
...
func (vq *VideoQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Video, error) {
	...
	if query := vq.withSubtitles; query != nil {
		if err := vq.loadSubtitles(ctx, query, nodes,
			func(n *Video) { n.Edges.Subtitles = []*Subtitle{} },
			func(n *Video, e *Subtitle) { n.Edges.Subtitles = append(n.Edges.Subtitles, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (vq *VideoQuery) loadSubtitles(ctx context.Context, query *SubtitleQuery, nodes []*Video, init func(*Video), assign func(*Video, *Subtitle)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(subtitle.FieldVideoID)
	}
	query.Where(predicate.Subtitle(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(video.SubtitlesColumn), fks...))
	}))
