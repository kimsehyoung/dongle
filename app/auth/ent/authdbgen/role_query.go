...
func (rq *RoleQuery) Exist(ctx context.Context) (bool, error) {
	...
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("authdbgen: check existence: %w", err)
	default:
		return true, nil
	}
}
...
//
//	client.Role.Query().
//		GroupBy(role.FieldType).
//		Aggregate(authdbgen.Count()).
//		Scan(ctx, &v)
...
func (rq *RoleQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range rq.inters {
		if inter == nil {
			return fmt.Errorf("authdbgen: uninitialized interceptor (forgotten import authdbgen/runtime?)")
		}
	...
	for _, f := range rq.ctx.Fields {
		if !role.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("authdbgen: invalid field %q for query", f)}
		}
	}
	if rq.path != nil {
