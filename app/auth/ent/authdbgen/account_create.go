...
func (ac *AccountCreate) check() error {
	if _, ok := ac.mutation.RoleID(); !ok {
		return &ValidationError{Name: "role_id", err: errors.New(`authdbgen: missing required field "Account.role_id"`)}
	}
	if _, ok := ac.mutation.Email(); !ok {
		return &ValidationError{Name: "email", err: errors.New(`authdbgen: missing required field "Account.email"`)}
	}
	if _, ok := ac.mutation.HashedPassword(); !ok {
		return &ValidationError{Name: "hashed_password", err: errors.New(`authdbgen: missing required field "Account.hashed_password"`)}
	}
	if _, ok := ac.mutation.Name(); !ok {
		return &ValidationError{Name: "name", err: errors.New(`authdbgen: missing required field "Account.name"`)}
	}
	if _, ok := ac.mutation.PhoneNumber(); !ok {
		return &ValidationError{Name: "phone_number", err: errors.New(`authdbgen: missing required field "Account.phone_number"`)}
	}
	if _, ok := ac.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`authdbgen: missing required field "Account.created_at"`)}
	}
	if _, ok := ac.mutation.RoleID(); !ok {
		return &ValidationError{Name: "role", err: errors.New(`authdbgen: missing required edge "Account.role"`)}
	}
	return nil
}
